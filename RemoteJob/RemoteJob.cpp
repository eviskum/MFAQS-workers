// RemoteJob.cpp : Defines the initialization routines for the DLL.
//
...
BEGIN_MESSAGE_MAP(CRemoteJobApp, CWinApp)
	//{{AFX_MSG_MAP(CRemoteJobApp)
...
/////////////////////////////////////////////////////////////////////////////
// CRemoteJobApp construction

CRemoteJobApp::CRemoteJobApp()
{
...
/////////////////////////////////////////////////////////////////////////////
// The one and only CRemoteJobApp object

CRemoteJobApp theApp;

/////////////////////////////////////////////////////////////////////////////
// CRemoteJobApp initialization

BOOL CRemoteJobApp::InitInstance()
{
	if (!AfxSocketInit())
	{
...
CString comment, remotehost, remotecmd;

int readconfig(char remotefile[])
{
	TCHAR tmp_str[1000];
	int returnval = TRUE;
	TRY
	{
		CStdioFile SQLFile(remotefile, CFile::modeRead);
		SQLFile.ReadString(comment);
		SQLFile.ReadString(remotehost);
		SQLFile.ReadString(remotecmd);
		SQLFile.Close();
	}
	CATCH(CFileException, e) {
		sprintf(tmp_str, "Kunne ikke hente remote-configurationsfil: '%s'", remotefile);
		LJERR(tmp_str)
		switch (e->m_cause) {
...
extern "C" int PASCAL EXPORT DllWorker ( int argc, char *argv[] )
{
	...
	LJNORM("Remote job aktiverings utility")

	if (argc != 1) {
		LJERR("Forkert brug !!!! Korrekt brug: REMOTEJOB configfile")
		return(DLLERROR);
	}
...
//	DeleteFile(argv[1]);
	...
	if (readconfig(argv[0])) {
		...
		sendersocket.Create();
		if (sendersocket.Connect((LPCSTR) remotehost, 12000)) {
			CSocketFile senderfile(&sendersocket);
			CArchive rarch(&senderfile, CArchive::load);
			CArchive sarch(&senderfile, CArchive::store);
			CString rtekst;
			sarch << remotecmd;
			sarch.Flush();
			rarch >> rtekst;
			LJNORM((LPCSTR) rtekst)
			rarch.Close(); sarch.Close();
			senderfile.Close();
			sendersocket.Close();
		} else {
			LJERR("Kunne ikke connecte til remote job maskine")
		}
	} else {
		return(DLLERROR);
	}
